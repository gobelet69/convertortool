...
var ConversionError, FORMAT_FILTERS, FORMAT_MIME_TYPES, EXTENSION_TO_FORMAT, LOK_DOCTYPE_OUTPUT_FORMATS, OUTPUT_FORMAT_TO_LOK, FORMAT_FILTER_OPTIONS, CSV_IMPORT_FILTER_OPTIONS, INPUT_FORMAT_CATEGORY, CATEGORY_OUTPUT_FORMATS;
var init_types = __esm({
  "src/types.ts"() {
    ...
    CSV_IMPORT_FILTER_OPTIONS = "FilterName=Text - txt - csv (StarCalc),FilterOptions=44,34,76,1,,1033,false,true,false,false,false,0,true,false,true";
...
var LOK_MOUSEEVENT_BUTTONDOWN, LOK_MOUSEEVENT_BUTTONUP, LOK_CALLBACK_INVALIDATE_TILES, LOK_CALLBACK_INVALIDATE_VISIBLE_CURSOR, LOK_CALLBACK_TEXT_SELECTION, LOK_CALLBACK_TEXT_SELECTION_START, LOK_CALLBACK_TEXT_SELECTION_END, LOK_CALLBACK_CURSOR_VISIBLE, LOK_CALLBACK_GRAPHIC_SELECTION, LOK_CALLBACK_HYPERLINK_CLICKED, LOK_CALLBACK_STATE_CHANGED, LOK_CALLBACK_STATUS_INDICATOR_START, LOK_CALLBACK_STATUS_INDICATOR_SET_VALUE, LOK_CALLBACK_STATUS_INDICATOR_FINISH, LOK_CALLBACK_SEARCH_NOT_FOUND, LOK_CALLBACK_DOCUMENT_SIZE_CHANGED, LOK_CALLBACK_SET_PART, LOK_CALLBACK_SEARCH_RESULT_SELECTION, LOK_CALLBACK_UNO_COMMAND_RESULT, LOK_CALLBACK_CELL_CURSOR, LOK_CALLBACK_MOUSE_POINTER, LOK_CALLBACK_CELL_FORMULA, LOK_CALLBACK_DOCUMENT_PASSWORD, LOK_CALLBACK_DOCUMENT_PASSWORD_TO_MODIFY, LOK_CALLBACK_CONTEXT_MENU, LOK_CALLBACK_INVALIDATE_VIEW_CURSOR, LOK_CALLBACK_TEXT_VIEW_SELECTION, LOK_CALLBACK_CELL_VIEW_CURSOR, LOK_CALLBACK_GRAPHIC_VIEW_SELECTION, LOK_CALLBACK_VIEW_CURSOR_VISIBLE, LOK_CALLBACK_VIEW_LOCK, LOK_CALLBACK_REDLINE_TABLE_SIZE_CHANGED, LOK_CALLBACK_REDLINE_TABLE_ENTRY_MODIFIED, LOK_CALLBACK_COMMENT, LOK_CALLBACK_INVALIDATE_HEADER, LOK_CALLBACK_CELL_ADDRESS, LOK_CLASS, DOC_CLASS, textEncoder, textDecoder, LOKBindings;
var init_lok_bindings = __esm({
  "src/lok-bindings.ts"() {
    LOK_MOUSEEVENT_BUTTONDOWN = 0;
    LOK_MOUSEEVENT_BUTTONUP = 1;
    ...
    LOK_CALLBACK_CONTEXT_MENU = 22;
    LOK_CALLBACK_INVALIDATE_VIEW_CURSOR = 23;
    LOK_CALLBACK_TEXT_VIEW_SELECTION = 24;
    LOK_CALLBACK_CELL_VIEW_CURSOR = 25;
    LOK_CALLBACK_GRAPHIC_VIEW_SELECTION = 26;
    LOK_CALLBACK_VIEW_CURSOR_VISIBLE = 27;
    LOK_CALLBACK_VIEW_LOCK = 28;
    LOK_CALLBACK_REDLINE_TABLE_SIZE_CHANGED = 29;
    LOK_CALLBACK_REDLINE_TABLE_ENTRY_MODIFIED = 30;
    LOK_CALLBACK_COMMENT = 31;
    LOK_CALLBACK_INVALIDATE_HEADER = 32;
    LOK_CALLBACK_CELL_ADDRESS = 33;
    LOK_CLASS = {
      nSize: 0,
      destroy: 4,
      documentLoad: 8,
    ...
    LOKBindings = class {
       ...
       */
      toFileUrl(path2) {
        if (path2.startsWith("file://")) return path2;
      ...
      documentLoad(path2) {
        ...
        const fileUrl = this.toFileUrl(path2);
        this.log("Loading document:", path2, "->", fileUrl);
       ...
       */
      documentLoadWithOptions(path2, options) {
        if (this.lokPtr === 0) {
          throw new Error("LOK not initialized");
        }
        const fileUrl = this.toFileUrl(path2);
        this.log("Loading document with options:", path2, "->", fileUrl, options);
        const pathPtr = this.allocString(fileUrl);
        ...
        try {
          ...
          if (this.useShims && this.module._lok_documentLoadWithOptions) {
            ...
            if (loadWithOptsPtr === 0) {
              return this.documentLoad(path2);
            }
...
var LibreOfficeConverter;
var init_converter_node = __esm({
  "src/converter-node.ts"() {
    init_types();
    init_lok_bindings();
    LibreOfficeConverter = class {
      module = null;
      lokBindings = null;
      initialized = false;
      initializing = false;
...
var init_base = __esm({
  "src/editor/base.ts"() {
    OfficeEditor = class {
      ...
      saveAs(path2, format) {
        try {
          this.lok.documentSaveAs(this.docPtr, path2, format, "");
          return this.createResult({ path: path2 });
        } catch (error) {
...
var init_writer = __esm({
  "src/editor/writer.ts"() {
    init_base();
    WriterEditor = class extends OfficeEditor {
...
var init_calc = __esm({
  "src/editor/calc.ts"() {
    init_base();
    CalcEditor = class extends OfficeEditor {
      getDocumentType() {
        return "calc";
      }
...
var ImpressEditor;
var init_impress = __esm({
  "src/editor/impress.ts"() {
    init_base();
    ImpressEditor = class extends OfficeEditor {
      getDocumentType() {
        return "impress";
      }
      getStructure(_options) {
...
var init_draw = __esm({
  "src/editor/draw.ts"() {
    init_base();
    DrawEditor = class extends OfficeEditor {
      isImportedPdf = false;
      getDocumentType() {
        return "draw";
      }
...
var LOK_DOCTYPE_TEXT, LOK_DOCTYPE_SPREADSHEET, LOK_DOCTYPE_PRESENTATION, LOK_DOCTYPE_DRAWING;
var init_editor = __esm({
  "src/editor/index.ts"() {
    ...
    init_draw();
    LOK_DOCTYPE_TEXT = 0;
    LOK_DOCTYPE_SPREADSHEET = 1;
    LOK_DOCTYPE_PRESENTATION = 2;
    LOK_DOCTYPE_DRAWING = 3;
  }
});
var require_subprocess_worker = __commonJS({
  "src/subprocess.worker.cts"() {
    init_converter_node();
    init_editor();
    init_types();
  ...
  Resolved to: ${wasmDir}`
    ...
    async function handleOpenDocument(payload) {
      ...
      const module2 = converter.getModule();
      if (!lokBindings || !module2) {
      ...
      const inputData = new Uint8Array(payload.inputData);
      module2.FS.writeFile(filePath, inputData);
    ...
    async function handleDestroy() {
      for (const [, session] of editorSessions) {
        try {
          ...
          if (module2) {
            try {
              module2.FS.unlink(session.filePath);
            } catch {
            }
          }
        } catch {
      ...
      try {
        ...
        switch (msg.type) {
          case "init":
            ...
            break;
          case "convert":
            result = await handleConvert(msg.payload);
            break;
          case "getPageCount":
            result = await handleGetPageCount(msg.payload);
            break;
          case "getDocumentInfo":
            result = await handleGetDocumentInfo(msg.payload);
            break;
          case "renderPage":
            result = await handleRenderPage(msg.payload);
            break;
          case "renderPagePreviews":
            result = await handleRenderPagePreviews(msg.payload);
            break;
          case "renderPageFullQuality":
            result = await handleRenderPageFullQuality(msg.payload);
            break;
          case "getDocumentText":
            result = await handleGetDocumentText(msg.payload);
            break;
          case "getPageNames":
            result = await handleGetPageNames(msg.payload);
            break;
          case "openDocument":
            result = await handleOpenDocument(msg.payload);
            break;
          case "editorOperation":
            result = await handleEditorOperation(msg.payload);
            break;
          case "closeDocument":
            result = await handleCloseDocument(msg.payload);
            break;
          case "destroy":
            await handleDestroy();
            break;
...
var subprocess_worker = require_subprocess_worker();

module.exports = subprocess_worker;
//# sourceMappingURL=subprocess.worker.cjs.map
//# sourceMappingURL=subprocess.worker.cjs.map